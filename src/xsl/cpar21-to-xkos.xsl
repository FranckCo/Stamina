...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="base-url">http://ec.europa.eu/eurostat/codes/cpar21/</xsl:variable>
    ...
    <xsl:template match="Classification">
        <rdf:Description rdf:about="{$base-url}cpa">
            ...
            <skos:altLabel xml:lang="en">CPA Rev. 2.1</skos:altLabel>
    ...
    <xsl:template match="Level">
        ...
        <xsl:param name="level-names" select="tokenize('Sections,Divisions,Groups,Classes,Categories,Sub-categories', ',')"/>
        <xsl:param name="level-patterns" select="tokenize('[A-U],[0-9]{2},[0-9]{2}\.[0-9],[0-9]{2}\.[0-9]{2},[0-9]{2}\.[0-9]{2}\.[0-9],[0-9]{2}\.[0-9]{2}\.[0-9]{2}', ',')"/>
        ...
        <xsl:for-each select="1 to $level-count">
            ...
            <rdf:Description rdf:about="{lf:level-uri($level-number)}">
                ...
                <skos:prefLabel xml:lang="en"><xsl:value-of select="concat('CPA Rev. 2.1 - Level ', $level-number, ' - ', $level-names[$level-number])"/></skos:prefLabel>
    ...
    <xsl:template match="Item">
        ...
        <rdf:Description rdf:about="{lf:item-uri($item-code)}">
            ...
            <!-- No explanatory notes for the CPA -->
    ...
    <xsl:function name="lf:level-uri" as="xs:string">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$level-depth=6"><xsl:value-of select="concat($base-url, 'sub-categories')"/></xsl:when>
    ...
    <xsl:function name="lf:level-concept-uri" as="xs:string">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$level-depth=6"><xsl:value-of select="concat($base-url, 'concept/sub-category')"/></xsl:when>
            <xsl:otherwise>unknown</xsl:otherwise>
        </xsl:choose>
    </xsl:function>
